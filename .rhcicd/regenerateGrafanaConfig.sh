...
YAML_FILE=`ls $SCRIPT_DIR | grep -m 1 grafana-dashboard-*.configmap.yaml`;
  [[ "$DEBUG_ENABLED" == "$YES" ]] && [[ DBG == "true" ]] && DebugLog "YAML_FILE" "$YAML_FILE" "(testing for expected config file)" "FILE";
# load constants
WRAPPER_YAML="wrapper.yaml";
  [[ "$DEBUG_ENABLED" == "$YES" ]] && [[ DBG == "true" ]] && DebugLog "WRAPPER_YAML" "$WRAPPER_YAML" "(testing for expected yaml wrapper file)" "FILE";
SECTIONS_JSON="sections.json";
  [[ "$DEBUG_ENABLED" == "$YES" ]] && [[ DBG == "true" ]] && DebugLog "SECTIONS_JSON" "$SECTIONS_JSON" "(testing for expected json sections file)" "FILE";
