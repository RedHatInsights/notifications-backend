...
@ApplicationScoped
public class TeamsProcessor extends EndpointTypeProcessor {

    public static final String PROCESSED_COUNTER_NAME = "processor.teams.processed";
    ...
    private void process(Event event, Endpoint endpoint) {
        ...
        Log.infof("Sending Teams notification through Camel [orgId=%s, eventId=%s, historyId=%s]",
                endpoint.getOrgId(), event.getId(), historyId);

        long startTime = System.currentTimeMillis();

        TeamsNotification notification = buildNotification(event, endpoint, historyId);
        ...
        try {
            ...
            Log.infof(e, "Sending Teams notification through Camel failed [eventId=%s, historyId=%s]", event.getId(), historyId);
        }
    ...
    private TeamsNotification buildNotification(Event event, Endpoint endpoint, UUID historyId) {
        ...
        try {
            ...
            throw new RuntimeException("Teams notification data transformation failed", e);
        }
    ...
    private TemplateInstance getTemplate(String orgId) {
        IntegrationTemplate integrationTemplate = templateRepository.findIntegrationTemplate(null, orgId, ORG, "teams")
