...
- Container base image digest updates (e.g. `ubi9/openjdk-21-runtime:latest`) are handled by Renovate's dockerfile manager, which pins each `FROM` line to `tag@sha256` and automerges digest updates.
