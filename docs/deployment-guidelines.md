...
- Base image digests (all `FROM` lines use `latest` tags, pinned to `tag@sha256`) are tracked and auto-merged by Renovate; the former daily `base-image-auto-update.yml` GitHub Action was removed.
