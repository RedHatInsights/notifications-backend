...
- **Renovate (Mintmaker)**: Configured in `renovate.jsonc`. MintMaker's global config applies to every onboarded repo, so this file only holds overrides. Tekton pipeline, Maven, npm, GitHub Actions (allow-listed), and base image digest updates automerge on patch/minor (Tekton and base image digests also automerge); Quarkus (`io.quarkus*`), the Maven wrapper, and every major bump stay on manual review.
- **Dependabot**: Removed -- Renovate/MintMaker now covers Maven, GitHub Actions, and npm. Dependabot security alerts remain enabled separately; only version updates were replaced.
