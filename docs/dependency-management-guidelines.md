...
- **Renovate (MintMaker)**: Configured in `renovate.jsonc`, which holds only this repository's overrides on top of MintMaker's global config. Targets the `master` branch. Maven, npm, Tekton pipeline, and container digest updates automerge (patch/minor, majors stay manual); `io.quarkus*` and the Maven wrapper always stay manual.
