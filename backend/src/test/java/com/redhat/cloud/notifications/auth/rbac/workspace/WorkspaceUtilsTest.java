...
public class WorkspaceUtilsTest {
    ...
    void testInvalidNonDefaultWorkspacesReturnedThrowsException() {
        ...
        final IllegalStateException multipleWorkspacesException = Assertions.assertThrows(
            IllegalStateException.class,
            () -> this.workspaceUtils.getDefaultWorkspaceId(DEFAULT_ORG_ID)
        );
        Assertions.assertEquals(WorkspaceUtils.ERROR_USER_FACING, multipleWorkspacesException.getMessage(), "unexpected user facing error in the received exception");

        MockServerConfig.clearRbacWorkspaces();
        MockServerConfig.addReturningSingleRootWorkspaceRbacEndpoint();

        final IllegalStateException nonDefaultWorkspaceException = Assertions.assertThrows(
            IllegalStateException.class,
            () -> this.workspaceUtils.getDefaultWorkspaceId(DEFAULT_ORG_ID)
        );
        Assertions.assertEquals(WorkspaceUtils.ERROR_USER_FACING, nonDefaultWorkspaceException.getMessage(), "unexpected user facing error in the received exception");
