...
public class TestOpenshiftAdvisorTemplate {
    ...
    public void testInstantEmailBody() {
        Action action = TestHelpers.createAdvisorOpenshiftAction("123456", "new-recommendation");
        final String result = AdvisorOpenshift.Templates.newRecommendationInstantEmailBody().data("action", action)
                .render();
        ...
        action.getEvents().forEach(event -> {
            ...
            assertTrue(result.contains(event.getPayload().get("total_risk").toString()),
                    "Body should contain total_risk" + event.getPayload().get("total_risk"));
            assertTrue(result.contains(event.getPayload().get("rule_url").toString()),
    ...
    public void testDigestEmailTitle() {
        ...
        String result = AdvisorOpenshift.Templates.weeklyDigestEmailTitle().data("action", action).render();
    ...
    public void testDigestEmailBody() {
        Action action = TestHelpers.createAdvisorOpenshiftAction("123456", "weekly-digest");
        final String result = AdvisorOpenshift.Templates.weeklyDigestEmailBody().data("action", action).render();

        assertTrue(action.getEvents().size() == 1, "weekly digest should only contain one event");

        action.getEvents().forEach(event -> {
            assertTrue(result.contains(event.getPayload().get("total_clusters").toString()),
                    "Body should contain total_clusters" + event.getPayload().get("total_clusters"));
            assertTrue(result.contains(event.getPayload().get("total_recommendations").toString()),
                    "Body should contain total_recommendations" + event.getPayload().get("total_recommendations"));
            assertTrue(result.contains(event.getPayload().get("total_incidents").toString()),
                    "Body should contain total_incidents" + event.getPayload().get("total_incidents"));
            assertTrue(result.contains(event.getPayload().get("total_critical").toString()),
                    "Body should contain total_critical" + event.getPayload().get("total_critical"));
            assertTrue(result.contains(event.getPayload().get("total_important").toString()),
