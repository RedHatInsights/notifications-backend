...
public class DrawerNotificationRepository {
    ...
    public Integer updateReadStatus(String orgId, String username, Set<UUID> notificationIds, Boolean readStatus) {
        ...
        String hql = "UPDATE DrawerNotification SET read = :readStatus "
            + "WHERE id.orgId = :orgId and id.userId = :userId and id.event.id in (:notificationIds)";
    ...
    public List<DrawerEntryPayload> getNotifications(String orgId, String username, Set<UUID> bundleIds, Set<UUID> appIds, Set<UUID> eventTypeIds,
        ...
        String hql = "SELECT dn.id.event.id, dn.read, " +
            "dn.id.event.bundleDisplayName, dn.id.event.applicationDisplayName, dn.id.event.eventTypeDisplayName, dn.created, dn.id.event.renderedDrawerNotification, bundle.name "
            + "FROM DrawerNotification dn join Bundle bundle on dn.id.event.bundleId = bundle.id where dn.id.orgId = :orgId and dn.id.userId = :userid";
    ...
    private static String addHqlConditions(String hql, Set<UUID> bundleIds, Set<UUID> appIds, Set<UUID> eventTypeIds,
        ...
        if (eventTypesNotEmpty ||
            ...
            hql += " AND dn.id.event.orgId = :orgId";

            if (eventTypesNotEmpty) {
                hql += " AND dn.id.event.eventType.id IN (:eventTypeIds)";
            } else if (applicationsNotEmpty) {
                hql += " AND dn.id.event.applicationId IN (:appIds)";
            } else if (bundlesNotEmpty) {
                hql += " AND dn.id.event.bundleId IN (:bundleIds)";
            }
        }
