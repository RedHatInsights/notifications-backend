...
public class BehaviorGroupResources {
    ...
    public Uni<BehaviorGroup> create(String accountId, BehaviorGroup behaviorGroup) {
        return sessionFactory.withSession(session -> {
            return session.find(Bundle.class, behaviorGroup.getBundleId()).onItem().ifNull()
                    .failWith(new NotFoundException("bundle_id not found")).onItem().invoke(bundle -> {
                        behaviorGroup.setBundle(bundle);
                        behaviorGroup.setAccountId(accountId);
                    }).replaceWith(session.persist(behaviorGroup)).call(session::flush).replaceWith(behaviorGroup)
    ...
    public Uni<List<BehaviorGroup>> findByBundleId(String accountId, UUID bundleId) {
        return sessionFactory.withSession(session -> {
            return session.createNamedQuery("findByBundleId", BehaviorGroup.class).setParameter("accountId", accountId)
                    .setParameter("bundleId", bundleId).getResultList().onItem()
                    .invoke(behaviorGroups -> behaviorGroups.forEach(BehaviorGroup::filterOutBundle));
        });
    }
    ...
    public Uni<Boolean> update(String accountId, BehaviorGroup behaviorGroup) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query).setParameter("displayName", behaviorGroup.getDisplayName())
                    .setParameter("accountId", accountId).setParameter("id", behaviorGroup.getId()).executeUpdate()
    ...
    public Uni<Boolean> delete(String accountId, UUID behaviorGroupId) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query).setParameter("accountId", accountId).setParameter("id", behaviorGroupId)
                    .executeUpdate().call(session::flush).onItem().transform(rowCount -> rowCount > 0);
        });
    }

    /*
     * Returns Boolean.TRUE if the event type was found and successfully updated. Returns Boolean.FALSE if the event
     * type was not found. If an exception other than NoResultException is thrown during the update, the DB transaction
     * will be rolled back.
     */
    public Uni<Boolean> updateEventTypeBehaviors(String accountId, UUID eventTypeId, Set<UUID> behaviorGroupIds) {
        return sessionFactory.withTransaction((session, tx) -> {
            ...
            return session.find(EventType.class, eventTypeId).onItem().ifNull().failWith(new NoResultException())
                    .onItem().call(() -> {

                        /*
                         * All event type behaviors that should no longer exist must be deleted. Deleted event type
                         * behaviors must obviously be owned by the current account.
                         */
                        String deleteQuery = "DELETE FROM EventTypeBehavior b " + "WHERE eventType.id = :eventTypeId "
                        ...
                        Mutiny.Query mutinyQuery = session.createQuery(deleteQuery).setParameter("accountId", accountId)
                    ...
                    }).onItem().transformToMulti(ignored -> Multi.createFrom().iterable(behaviorGroupIds)).onItem()
                    .transformToUniAndConcatenate(behaviorGroupId -> {

                        /*
                         * Then, we'll insert all event type behaviors from the given behaviorGroupIds list. If an event
                         * type behavior already exists, nothing will happen (no exception will be thrown).
                         */
                        ...
                        return session.createNativeQuery(insertQuery).setParameter("eventTypeId", eventTypeId)
                                .setParameter("behaviorGroupId", behaviorGroupId)
                                .setParameter("created", LocalDateTime.now(UTC)).setParameter("accountId", accountId)
                                .executeUpdate();

                    }).collect().asList().replaceWith(Boolean.TRUE)
    ...
    public Uni<List<EventType>> findEventTypesByBehaviorGroupId(String accountId, UUID behaviorGroupId) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query, EventType.class).setParameter("accountId", accountId)
                    .setParameter("behaviorGroupId", behaviorGroupId).getResultList();
        });
    }
    ...
    public Uni<List<BehaviorGroup>> findBehaviorGroupsByEventTypeId(String accountId, UUID eventTypeId, Query limiter) {
        return sessionFactory.withSession(session -> {
            ...
            Mutiny.Query<BehaviorGroup> mutinyQuery = session.createQuery(query, BehaviorGroup.class)
                    .setParameter("accountId", accountId).setParameter("eventTypeId", eventTypeId);
            ...
            return mutinyQuery.getResultList().onItem().invoke(behaviorGroups -> behaviorGroups
                    .forEach(behaviorGroup -> behaviorGroup.filterOutBundle().filterOutActions()));
        });
    }

    /*
     * Returns Status.OK if the behavior group was found and successfully updated. Returns Status.NOT_FOUND if the
     * behavior group was not found. Returns Status.BAD_REQUEST if the behavior group contains more than one
     * EMAIL_SUBSCRIPTION action. If an exception other than NoResultException is thrown during the update, the DB
    ...
    public Uni<Status> updateBehaviorGroupActions(String accountId, UUID behaviorGroupId, List<UUID> endpointIds) {
        return sessionFactory.withTransaction((session, tx) -> {
            ...
            return session.createQuery(checkBehaviorGroupQuery).setParameter("accountId", accountId)
                    .setParameter("id", behaviorGroupId).getSingleResult().onItem().call(() -> {
                        ...
                        Mutiny.Query mutinyQuery = session.createQuery(deleteQuery).setParameter("behaviorGroupId",
                                behaviorGroupId);
                        if (!endpointIds.isEmpty()) {
                    ...
                    }).onItem().transformToMulti(ignored -> Multi.createFrom().iterable(endpointIds)).onItem()
                    .transformToUniAndConcatenate(endpointId -> {

                        /*
                         * Then, we'll execute an "upsert" based on the given endpointIds list: - if an action already
                         * exists, its position will be updated - otherwise, the action will be inserted into the
                         * database In the end, all inserted or updated actions will have the same position than the
                         * endpointIds list order.
                         */
                        ...
                        return session.createNativeQuery(upsertQuery).setParameter("behaviorGroupId", behaviorGroupId)
                                .setParameter("endpointId", endpointId)
                                .setParameter("position", endpointIds.indexOf(endpointId))
                                .setParameter("created", LocalDateTime.now(UTC)).setParameter("accountId", accountId)
                                .executeUpdate();

                    }).collect().asList().replaceWith(OK)
                    ...
                    // The following exception will be thrown if the behavior group contains more than one
                    // EMAIL_SUBSCRIPTION action.
    ...
    public Uni<List<BehaviorGroup>> findBehaviorGroupsByEndpointId(String accountId, UUID endpointId) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query, BehaviorGroup.class).setParameter("accountId", accountId)
                    .setParameter("endpointId", endpointId).getResultList().onItem()
