...
public class EmailAggregator {
    ...
    private Uni<Set<String>> getEmailSubscribers(EmailAggregationKey aggregationKey,
            EmailSubscriptionType emailSubscriptionType) {
        return subscriptionResources
                .getEmailSubscribersUserId(aggregationKey.getAccountId(), aggregationKey.getBundle(),
                        aggregationKey.getApplication(), emailSubscriptionType)
                .onItem().transform(Set::copyOf)
                // This will prevent multiple database queries executions during the aggregation process.
                .memoize().indefinitely();
    }

    public Uni<Map<User, Map<String, Object>>> getAggregated(EmailAggregationKey aggregationKey,
            EmailSubscriptionType emailSubscriptionType, LocalDateTime start, LocalDateTime end) {
        ...
        return emailAggregationResources.getEmailAggregation(aggregationKey, start, end).onItem()
                ...
                .onItem().transformToUniAndConcatenate(aggregation -> {
                    ...
                    return endpointResources
                            .getTargetEndpointsFromType(aggregationKey.getAccountId(), aggregationKey.getBundle(),
                                    aggregationKey.getApplication(), eventType, EndpointType.EMAIL_SUBSCRIPTION)
                            .onItem().transform(Set::copyOf)
                            // Now we want to determine who will actually receive the aggregation email.
                            .onItem().transformToUni(endpoints ->
                    // All users who subscribed to the current application and subscription type combination are
                    // recipients candidates.
                    subscribers.onItem().transformToUni(users ->
                    /*
                     * The actual recipients list is a subset of the candidates. The target endpoints properties will
                     * determine whether or not each candidate will actually receive an email.
                     */
                    recipientResolver.recipientUsers(aggregationKey.getAccountId(), endpoints, users)))
                            /*
                             * We now have the final recipients list. Let's populate with synchronized side-effects
                             * (invoke) the Map that will be returned by the method.
                             */
                            .onItem().invoke(users -> {
                                users.forEach(user -> {
                ...
                .replaceWith(aggregated).onItem()
                .transform(aggregatorMap -> aggregatorMap.entrySet().stream().peek(entry -> {
                    // TODO These fields could be passed to EmailPayloadAggregatorFactory.by since we know them from the
                    // beginning.
    ...
    private void fillUsers(EmailAggregationKey aggregationKey, User user,
            Map<User, AbstractEmailPayloadAggregator> aggregated, EmailAggregation emailAggregation) {
        AbstractEmailPayloadAggregator aggregator = aggregated.computeIfAbsent(user,
                ignored -> EmailPayloadAggregatorFactory.by(aggregationKey));
