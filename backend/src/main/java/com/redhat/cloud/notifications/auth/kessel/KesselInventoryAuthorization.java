...
public class KesselInventoryAuthorization {
     ...
     */
    public void hasPermissionOnResource(final SecurityContext securityContext, final KesselPermission permission, final KesselResourceType resourceType, final String resourceId) {
    ...
    private void checkReadOnly(KesselPermission permission, KesselResourceType resourceType, String resourceId, RhIdentity identity) {
        ...
        try {
            response = this.kesselCheckClient.check(permissionCheckRequest);
    ...
    private void checkForUpdate(KesselPermission permission, KesselResourceType resourceType, String resourceId, RhIdentity identity) {
        // Build the request for Kessel.
        ...
        try {
            response = this.kesselCheckClient.checkForUpdate(permissionCheckRequest);
        } catch (final Exception e) {
            Log.errorf(
                e,
                "[identity: %s][permission: %s][resource_type: %s][resource_id: %s] Unable to query Kessel for a permission on a resource",
    ...
    public void hasPermissionOnWorkspace(final SecurityContext securityContext, final WorkspacePermission workspacePermission, final UUID workspaceId) {
        this.hasPermissionOnResource(securityContext, workspacePermission, KesselResourceType.WORKSPACE, workspaceId.toString());
    }

    private ResourceReference buildObjectReference(KesselResourceType resourceType, String resourceId) {
    ...
    private CheckRequest buildCheckRequest(RhIdentity identity, KesselPermission permission, KesselResourceType resourceType, String resourceId) {
    ...
    private CheckForUpdateRequest buildCheckForUpdateRequest(RhIdentity identity, KesselPermission permission, KesselResourceType resourceType, String resourceId) {
    ...
    CheckOperation getCheckOperation(KesselResourceType resourceType, KesselPermission permission) {
        if ((resourceType == KesselResourceType.WORKSPACE) && (permission instanceof WorkspacePermission workspacePermission)) {
