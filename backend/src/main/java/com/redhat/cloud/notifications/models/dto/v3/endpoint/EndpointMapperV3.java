...
public interface EndpointMapperV3 {
    /**
    ...
    @Mapping(target = "secretToken", ignore = true)
    @Mapping(target = "secretTokenSourcesId", ignore = true)
    // v3 does not expose the HTTP method: all webhooks are forced to POST anyway (see the
    // V1.114.0__RHCLOUD-32168_set_webhook_method_post.sql migration), so it is hardcoded here.
    @Mapping(target = "method", constant = "POST")
