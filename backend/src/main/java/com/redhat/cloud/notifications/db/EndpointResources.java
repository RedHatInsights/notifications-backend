...
public class EndpointResources {
    ...
    public Uni<Endpoint> createEndpoint(Endpoint endpoint) {
        return sessionFactory.withSession(session -> {
            return session.persist(endpoint).onItem().call(session::flush).onItem().call(() -> {
                // If the endpoint properties are null, they won't be persisted.
                if (endpoint.getProperties() != null) {
                    /*
                     * As weird as it seems, we need the following line because the Endpoint instance was deserialized
                     * from JSON and that JSON did not contain any information about the
                     * 
                    ...
                    case EMAIL_SUBSCRIPTION:
                        return session.persist(endpoint.getProperties()).onItem().call(session::flush);
                    default:
                        // Do nothing.
                        break;
                    }
                ...
                /*
                 * If this line is reached, it means the endpoint properties are null or we don't support persisting
                 * properties for the endpoint type. We still have to return something.
                 */
    ...
    public Uni<List<Endpoint>> getEndpointsPerType(String tenant, EndpointType type, Boolean activeOnly,
            Query limiter) {
        return sessionFactory.withSession(session -> {
            // TODO Modify the parameter to take a vararg of Functions that modify the query
            // TODO Modify to take account selective joins (JOIN (..) UNION (..)) based on the type, same for
            // getEndpoints
            ...
            Mutiny.Query<Endpoint> mutinyQuery = session.createQuery(query, Endpoint.class)
                    .setParameter("accountId", tenant).setParameter("endpointType", type);
            ...
            return mutinyQuery.getResultList().onItem().call(this::loadProperties);
        });
    }
    ...
    public Uni<EndpointType> getEndpointTypeById(String accountId, UUID endpointId) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query, EndpointType.class).setParameter("accountId", accountId)
                    .setParameter("endpointId", endpointId).getSingleResultOrNull();
        });
    }

    public Uni<Endpoint> getOrCreateEmailSubscriptionEndpoint(String accountId,
            EmailSubscriptionProperties properties) {
        return sessionFactory.withSession(session -> {
            return getEndpointsPerType(accountId, EndpointType.EMAIL_SUBSCRIPTION, null, null).onItem()
                    .call(this::loadProperties).onItem().transformToUni(emailEndpoints -> {
                        Optional<Endpoint> endpointOptional = emailEndpoints.stream()
                                .filter(endpoint -> properties
                                        .hasSameProperties(endpoint.getProperties(EmailSubscriptionProperties.class)))
                                .findFirst();
    ...
    public Uni<Long> getEndpointsCountPerType(String tenant, EndpointType type, Boolean activeOnly) {
        return sessionFactory.withSession(session -> {
            ...
            Mutiny.Query<Long> mutinyQuery = session.createQuery(query, Long.class).setParameter("accountId", tenant)
    ...
    public Uni<List<Endpoint>> getTargetEndpoints(String tenant, EventType eventType) {
        ...
        return sessionFactory.withStatelessSession(statelessSession -> {
            return statelessSession.createQuery(query, Endpoint.class).setParameter("eventType", eventType)
                    .setParameter("accountId", tenant).getResultList().onItem()
                    .call(endpoints -> loadProperties(endpoints, true));
        });
    }

    // Note: This method uses a stateless session
    public Uni<List<Endpoint>> getTargetEndpointsFromType(String tenant, String bundleName, String applicationName,
            String eventTypeName, EndpointType endpointType) {
        String query = "SELECT DISTINCT e FROM Endpoint e JOIN e.behaviorGroupActions bga JOIN bga.behaviorGroup.behaviors b "
        ...
        return sessionFactory.withStatelessSession(statelessSession -> {
            return statelessSession.createQuery(query, Endpoint.class).setParameter("applicationName", applicationName)
                    .setParameter("eventTypeName", eventTypeName).setParameter("accountId", tenant)
                    .setParameter("bundleName", bundleName).setParameter("endpointType", endpointType).getResultList()
    ...
    public Uni<List<Endpoint>> getEndpoints(String tenant, Query limiter) {
        return sessionFactory.withSession(session -> {
            // TODO Add the ability to modify the getEndpoints to return also with JOIN to
            // application_eventtypes_endpoints link table
            ...
            Mutiny.Query<Endpoint> mutinyQuery = session.createQuery(query, Endpoint.class).setParameter("accountId",
                    tenant);
    ...
    public Uni<Long> getEndpointsCount(String tenant) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query, Long.class).setParameter("accountId", tenant).getSingleResult();
        });
    }
    ...
    public Uni<Endpoint> getEndpoint(String tenant, UUID id) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query, Endpoint.class).setParameter("id", id).setParameter("accountId", tenant)
                    .getSingleResultOrNull().onItem().ifNotNull().transformToUni(endpoint -> loadProperties(endpoint));
        });
    }
    ...
    public Uni<Boolean> deleteEndpoint(String tenant, UUID id) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query).setParameter("id", id).setParameter("accountId", tenant).executeUpdate()
                    .call(session::flush).onItem().transform(rowCount -> rowCount > 0);
        });
    ...
    private Uni<Boolean> modifyEndpointStatus(String tenant, UUID id, boolean enabled) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(query).setParameter("id", id).setParameter("accountId", tenant)
                    .setParameter("enabled", enabled).executeUpdate().call(session::flush).onItem()
    ...
    public Uni<Boolean> updateEndpoint(Endpoint endpoint) {
        ...
        return sessionFactory.withSession(session -> {
            return session.createQuery(endpointQuery).setParameter("name", endpoint.getName())
                    .setParameter("description", endpoint.getDescription())
                    .setParameter("enabled", endpoint.isEnabled()).setParameter("accountId", endpoint.getAccountId())
                    .setParameter("id", endpoint.getId()).executeUpdate().call(session::flush).onItem()
                    .transformToUni(endpointRowCount -> {
                        if (endpointRowCount == 0) {
                            ...
                            case WEBHOOK:
                                ...
                                return session.createQuery(webhookQuery).setParameter("url", properties.getUrl())
                                        ...
                                        .setParameter("endpointId", endpoint.getId()).executeUpdate()
                                        .call(session::flush).onItem().transform(rowCount -> rowCount > 0);
                            case CAMEL:
                                CamelProperties cAttr = (CamelProperties) endpoint.getProperties();
                                return session.createQuery(camelQuery).setParameter("url", cAttr.getUrl())
                                        ...
                                        .setParameter("subType", endpoint.getType()).executeUpdate()
                                        .call(session::flush).onItem().transform(rowCount -> rowCount > 0);
                            default:
                                return Uni.createFrom().item(Boolean.TRUE);
                            }
    ...
    public Uni<Void> loadProperties(List<Endpoint> endpoints, boolean useStatelessSession) {
        ...
        return this.loadTypedProperties(WebhookProperties.class, endpointSet, EndpointType.WEBHOOK, useStatelessSession)
                .chain(() -> loadTypedProperties(CamelProperties.class, endpointSet, EndpointType.CAMEL,
                        useStatelessSession))
                .chain(() -> loadTypedProperties(EmailSubscriptionProperties.class, endpointSet,
                        EndpointType.EMAIL_SUBSCRIPTION, useStatelessSession));
    }

    private <T extends EndpointProperties> Uni<Void> loadTypedProperties(Class<T> typedEndpointClass,
            Set<Endpoint> endpoints, EndpointType type, boolean useStatelessSession) {
        Map<UUID, Endpoint> endpointsMap = endpoints.stream().filter(e -> e.getType().equals(type))
                .collect(Collectors.toMap(Endpoint::getId, Function.identity()));

        if (endpointsMap.size() > 0) {
            return find(typedEndpointClass, endpointsMap.keySet(), useStatelessSession).onItem()
    ...
    private <T extends EndpointProperties> Uni<List<T>> find(Class<T> typedEndpointClass, Set<UUID> endpointIds,
            boolean useStatelessSession) {
        if (useStatelessSession) {
            String query = "FROM " + typedEndpointClass.getSimpleName() + " WHERE id IN (:endpointIds)";
            return sessionFactory.withStatelessSession(statelessSession -> {
                return statelessSession.createQuery(query, typedEndpointClass).setParameter("endpointIds", endpointIds)
                        .getResultList();
            });
        } else {
            return sessionFactory.withSession(session -> {
    ...
    public Uni<Endpoint> loadProperties(Endpoint endpoint) {
        if (endpoint == null) {
            LOGGER.warning(
                    "Endpoint properties loading attempt with a null endpoint. It should never happen, this is a bug.");
            return Uni.createFrom().nullItem();
        }
        return this.loadProperties(Collections.singletonList(endpoint)).replaceWith(endpoint);
    }
}
