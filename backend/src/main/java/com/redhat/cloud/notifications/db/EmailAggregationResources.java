...
public class EmailAggregationResources {
    ...
    public Uni<Boolean> addEmailAggregation(EmailAggregation aggregation) {
        ...
        return sessionFactory.withStatelessSession(statelessSession -> {
            return statelessSession.insert(aggregation).replaceWith(Boolean.TRUE).onFailure()
                    .recoverWithItem(Boolean.FALSE);
        });
    }

    public Uni<List<EmailAggregationKey>> getApplicationsWithPendingAggregation(LocalDateTime start,
            LocalDateTime end) {
        ...
        return sessionFactory.withStatelessSession(statelessSession -> {
            return statelessSession.createQuery(query, EmailAggregationKey.class).setParameter("start", start)
                    .setParameter("end", end).getResultList();
        });
    }

    public Uni<List<EmailAggregation>> getEmailAggregation(EmailAggregationKey key, LocalDateTime start,
            LocalDateTime end) {
        ...
        return sessionFactory.withStatelessSession(statelessSession -> {
            return statelessSession.createQuery(query, EmailAggregation.class)
                    .setParameter("accountId", key.getAccountId()).setParameter("bundleName", key.getBundle())
                    .setParameter("applicationName", key.getApplication()).setParameter("start", start)
                    .setParameter("end", end).getResultList();
        });
    }
    ...
    public Uni<Integer> purgeOldAggregation(EmailAggregationKey key, LocalDateTime lastUsedTime) {
        ...
        return sessionFactory.withStatelessSession(statelessSession -> {
            return statelessSession.createQuery(query).setParameter("accountId", key.getAccountId())
                    .setParameter("bundleName", key.getBundle()).setParameter("applicationName", key.getApplication())
                    .setParameter("created", lastUsedTime).executeUpdate();
        });
    }
}
