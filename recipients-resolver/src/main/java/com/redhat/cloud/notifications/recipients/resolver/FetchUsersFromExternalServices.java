...
import com.redhat.cloud.notifications.recipients.resolver.rbac.RbacServiceToServiceOidc;
...
public class FetchUsersFromExternalServices {
    ...
    @Inject
    @RestClient
    RbacServiceToServicePsk rbacServiceToServicePsk;

    @Inject
    @RestClient
    RbacServiceToServiceOidc rbacServiceToServiceOidc;
    ...
    private RbacServiceToService getRbacClient(String orgId) {
        if (recipientsResolverConfig.isRbacOidcAuthEnabled(orgId)) {
            ...
            Log.debug("Using PSK RBAC client");
            return rbacServiceToServicePsk;
        }
    }
    ...
    List<User> transformItUserToUser(List<ITUserResponse> itUserResponses) {
        ...
        for (ITUserResponse itUserResponse : itUserResponses) {
            ...
            if (itUserResponse.accountRelationships.get(0).emails == null) {
                Log.infof("Skipping user account id: %s with username: %s because it doesn't have any email addresses", user.getId(), user.getUsername());
            } else {
