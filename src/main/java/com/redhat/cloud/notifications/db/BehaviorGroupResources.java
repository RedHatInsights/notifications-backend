...
public class BehaviorGroupResources {
    ...
    public Uni<List<BehaviorGroup>> findByBundleId(String accountId, UUID bundleId) {
        // When PostgreSQL sorts a BOOLEAN column in ASC order, true comes first. That's not true for all DBMS.
        String query = "FROM BehaviorGroup WHERE accountId = :accountId AND bundle.id = :bundleId ORDER BY defaultBehavior ASC, created DESC";
