...
class NotificationsAppTest {
    ...
    final String regex = ".*(/health|/metrics) HTTP/[0-9].[0-9]\\\" 200.*";
    ...
    void shouldMachWhenInputEndsWithNewLine() {
        assertTrue(Pattern.matches(regex, input.trim()));
    }
    ...
    void shouldMatchWhenHttpVersionIsTwo() {
        ...
        assertTrue(Pattern.matches(regex, inputWithHttpTwoZero.trim()));
    }
    ...
    void shouldNotMatchWithSomethingelse() {
        ...
        assertFalse(Pattern.matches(regex, inputWithHttpTwoZero.trim()));
    }
}
