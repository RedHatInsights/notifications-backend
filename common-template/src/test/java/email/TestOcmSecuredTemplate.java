...
@QuarkusTest
public class TestOcmSecuredTemplate extends EmailTemplatesRendererHelper {
    ...
    public void testUpgradeEmailTitle() {
        Action action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject", null, Optional.empty());
    ...
    public void testUpgradeScheduledInstantEmailBody() {
        Action action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject", "Upgrade scheduled", Optional.of(Map.of("template_sub_type", "upgrade-scheduled-template")));
        ...
        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject", "Upgrade scheduled", Optional.of(Map.of("template_sub_type", "upgrade-scheduled-template-rosa-hcp")));
        result = generateEmailBody(CLUSTER_UPDATE, action);
        assertTrue(result.contains(COMMON_SECURED_LABEL_CHECK));
        assertTrue(result.contains(TestHelpers.HCC_LOGO_TARGET));
        assertTrue(result.contains(OCM_TEST_ENV_URL));
    ...
    public void testUpgradeEndedInstantEmailBody() {
        Action action = OcmTestHelpers.createOcmAction("Dummy cluster name", "MOA", "<b>Dummy server name</b> is ready to go", "Awesome subject", null, Optional.of(Map.of("template_sub_type", "upgrade-ended-template")));
        String result = generateEmailBody(CLUSTER_UPDATE, action);
        assertTrue(result.contains(TestHelpers.HCC_LOGO_TARGET));
        assertTrue(result.contains(COMMON_SECURED_LABEL_CHECK));
        ...
        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "MOA", "<b>Dummy server name</b> is ready to go", "Awesome subject", null, Optional.of(additionalMapParameters));
        result = generateEmailBody(CLUSTER_UPDATE, action);
        assertFalse(result.contains("Check these resources for more information"));

        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "MOA", "<b>Dummy server name</b> is ready to go", "Awesome subject", null, Optional.of(Map.of("template_sub_type", "osd-trial-deletion-template", "doc_references", List.of("https://docs.redhat.com/en/documentation/red_hat_openshift_service_on_aws_classic_architecture/4/html/red_hat_openshift_cluster_manager/ocm-overview", "https://console.redhat.com/openshift"))));
        result = generateEmailBody(CLUSTER_UPDATE, action);
        assertTrue(result.contains("Check these resources for more information"));
        assertTrue(result.contains("https://docs.redhat.com/en/documentation/red_hat_openshift_service_on_aws_classic_architecture/4/html/red_hat_openshift_cluster_manager/ocm-overview"));
        assertTrue(result.contains("https://console.redhat.com/openshift"));
    ...
    public void testApprovedAccessEmailBody() {
        ...
        Action action = OcmTestHelpers.createOcmAction("Dummy cluster name", "MOA", "<b>Dummy server name</b> need a revision", "Awesome subject", null, Optional.of(Map.of("template_sub_type", "ocm-approved-access-template")));
    ...
    public void testClusterLifecycleInstantEmailBody() {
        // test generic template case
        Action action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSD", "<b>Dummy server name</b> need a revision", "Awesome subject");
        ...
        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject");
        ...
        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject", "Trial creation", Optional.of(Map.of("template_sub_type", "osd-trial-creation-template")));
        result = generateEmailBody(CLUSTER_LIFECYCLE, action);
        assertTrue(result.contains(COMMON_SECURED_LABEL_CHECK));
        assertTrue(result.contains(TestHelpers.HCC_LOGO_TARGET));
        ...
        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject", "Trial reminder", Optional.of(Map.of("template_sub_type", "osd-trial-reminder-template")));
        result = generateEmailBody(CLUSTER_LIFECYCLE, action);
        assertTrue(result.contains(COMMON_SECURED_LABEL_CHECK));
        assertTrue(result.contains(TestHelpers.HCC_LOGO_TARGET));
        ...
        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject", "Trial delete", Optional.of(Map.of("template_sub_type", "osd-trial-deletion-template")));
        result = generateEmailBody(CLUSTER_LIFECYCLE, action);
        assertTrue(result.contains(COMMON_SECURED_LABEL_CHECK));
        assertTrue(result.contains(TestHelpers.HCC_LOGO_TARGET));
        ...
        additionalMapParameters.put("doc_references", null);
        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject", "Trial delete", Optional.of(additionalMapParameters));
        result = generateEmailBody(CLUSTER_LIFECYCLE, action);
        assertFalse(result.contains("Check these resources for more information"));

        action = OcmTestHelpers.createOcmAction("Dummy cluster name", "OSDTrial", "<b>Dummy server name</b> need a revision", "Awesome subject", "Trial delete", Optional.of(Map.of("template_sub_type", "osd-trial-deletion-template", "doc_references", List.of("https://docs.redhat.com/en/documentation/red_hat_openshift_service_on_aws_classic_architecture/4/html/red_hat_openshift_cluster_manager/ocm-overview", "https://console.redhat.com/openshift"))));
        result = generateEmailBody(CLUSTER_LIFECYCLE, action);
        assertTrue(result.contains("Check these resources for more information"));
        assertTrue(result.contains("https://docs.redhat.com/en/documentation/red_hat_openshift_service_on_aws_classic_architecture/4/html/red_hat_openshift_cluster_manager/ocm-overview"));
        assertTrue(result.contains("https://console.redhat.com/openshift"));
