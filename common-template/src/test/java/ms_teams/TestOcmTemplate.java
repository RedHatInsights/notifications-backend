...
class TestOcmTemplate {
    ...
    String renderTemplate(final String eventType, final JsonObject message) {
        TemplateDefinition templateConfig = new TemplateDefinition(MS_TEAMS, "openshift", "cluster-manager", eventType);
    ...
    private void checkResult(String eventType, String result) {
        switch (eventType) {
            case null -> assertEquals("{\"text\":\"1 event triggered from Cluster Manager - OpenShift. [Open Cluster Manager](" + CLUSTER_MANAGER_DEFAULT_EVENT_URL + ")\"}", result);
